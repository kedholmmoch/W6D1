def it_was_ok
  ...
  Movie.select(:id, :title, :score).where("movies.score BETWEEN 2 AND 3")
end
...
def harrison_ford
  ...
  Movie.joins(:castings, :actors).where(actors: {name: 'Harrison Ford'}).where("castings_movies_join.ord != 1").select("movies.id, movies.title").distinct
